/*
	...
	Assignement: 6, AcrossWords with Undo
	
	This program is a combination of Professor Reed's solution for program 5 from this semester and program 6 from last semester
*/
...
void displayIdentifyingInformation()
{
    ...
    printf("Program: #6, AcrossWords w/ Undo   \n");
    printf("TA: Nianzu Ma, Tue 5pm \n");
    printf("Apr 26, 2017               \n");
    printf("\n");
...
//--------------------------------------------------------------------------------
//Taken from Dale Reed's solution for coinswap
...
//    scanf("%d", &dictionarySize);
    ...
    switch( dictionarySize) {
        case 1: strcpy( dictionaryName, SMALL_DICTIONARY_NAME); break;
        case 2: strcpy( dictionaryName, MEDIUM_DICTIONARY_NAME); break;
        case 3: strcpy( dictionaryName, LARGE_DICTIONARY_NAME); break;
        default: printf("Invalid dictionary size chosen.  Exiting program...\n");
            exit( -1);
            break;
    }
...
int main() {
    ...
    while( ! done( board, dictionary, numberOfWords)) {
        ...
        if( rowOrColumn == 'x') {
            ...
            displayBoard( board, pHead);
            rowOrColumn = ' ';  // reset userInput, so it doesn't trigger undo again
